/*  Energy monitor for 3-phase
    ...
    Temp fault codes: 300 deg = Faulty sensor, sensor broken or disconnected.
...
#define UNUSED_TEMPERATURE 30100
#define OUTOFRANGE_TEMPERATURE 30200
...
int readTemperature()
{
  ...
  if (result <= TEMP_RANGE_LOW || result >= TEMP_RANGE_HIGH)
    return OUTOFRANGE_TEMPERATURE;     // return value = 302 deg ('Out of range')
  return result;
}
/*
    Temp fault codes: BAD_TEMPERATURE          300 deg = Faulty sensor, sensor broken or disconnected.
                      UNUSED_TEMPERATURE       301 deg = Sensor has never been detected since power-up/reset.
                      OUTOFRANGE_TEMPERATURE   302 deg = Sensor returned an out-of-range value.
                                               85  deg   although in range, might indicate a wiring fault.
*/
