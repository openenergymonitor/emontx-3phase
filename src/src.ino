...
// The type of Radio Module, or none.
#define RFM69CW                                  // Can be RFM69CW
//#define SERIALOUT                                //   or SERIALOUT if a wired serial connection is used
//#define EMONESP                                  //   or EMONESP if an ESP WiFi module is used
...
int nodeID = 1;                                  //  node ID for this emonTx.
...
//   for the EU adapter use 241.2, for the USA adapter use 130.00
#define VCAL_EU 249.1;     // can use DIP switch 2 to set this as the starting value.

#ifdef EMONTX_SHIELD
double i1Cal = 60.61;     // iCal calculated value is 100A:0.05A for transformer / 22 Ohms for resistor = 90.91, or 60.61 for emonTx Shield
double i2Cal = 60.61;
double i3Cal = 60.61;
double i4Cal = 60.61;
#else
...
double i4Lead = 0.20;     // degrees that the v.t. phase error leads the c.t.4 phase error by, set to 2.00 for TxShield with extra 100A clamp.
...
#ifdef EMONTX_SHIELD
#define SUPPLY_VOLTS 4.97  // Important note regarding the TxShield v2.x:
...
// This will significantly alter all measured readings ofg current and voltage, and for the shield the 5V input is used as the ADC top reference and sets Vdiv offsets for the input channels..
// A typical value for an Uno + TxShield will be 5.00V minus 0.03V as there's a voltage drop accross the 8ohm current limiting input resistor.
#endif
...
float LOOPTIMESECONDS = LOOPTIME / 1000;
...
#define RFMIRQPIN 2   // 2 for Uno, 3 for Leonardo
#define SDOPIN 12
#define W1PIN 4       // 1-Wire pin for temperature
#define DS18B20_PWR 6  // Power for 1-wire temperature sensor, not connected on shield but needs define for sketch to run.
...
#ifdef TEMPERATURE_SENSING
typedef struct {
  int power1, power2, power3, power4, Vrms, temp[MAXONEWIRE] = {30100, 30100, 30100, 30100, 30100, 30100};
  unsigned long pulseCount;
} PayloadTx;
#else
typedef struct {
  int Vrms, power1, power2, power3, powFactor1, powFactor2, powFactor3, Wh1, Wh2, Wh3;
} PayloadTx;
#endif
PayloadTx emontx;
...
//added by DB 31.07.2018
double Wh1_accumulator = 0.0;
double Wh2_accumulator = 0.0;
double Wh3_accumulator = 0.0;
double Wh4_accumulator = 0.0;
double Ws1_calculated = 0.0;
double Ws2_calculated = 0.0;
double Ws3_calculated = 0.0;
double Ws4_calculated = 0.0;
//
...
void calculateVIPF()
{
  ...
  int powerFactor1pre = powerFactor1 * 100;//added by DB 31.07.2018
  int powerFactor2pre = powerFactor2 * 100;//added by DB 31.07.2018
  int powerFactor3pre = powerFactor3 * 100;//added by DB 31.07.2018
#ifdef CT4Phase
  int powerFactor4pre = powerFactor4 * 100;//added by DB 31.07.2018
#endif

  emontx.powFactor1 = (int)(powerFactor1pre + 0.5); //added by DB 31.07.2018
  emontx.powFactor2 = (int)(powerFactor2pre + 0.5); //added by DB 31.07.2018
  emontx.powFactor3 = (int)(powerFactor3pre + 0.5); //added by DB 31.07.2018
#ifdef CT4Phase
  emontx.powFactor4 = (int)(powerFactor4pre + 0.5); //added by DB 31.07.2018
#endif

  Ws1_calculated = LOOPTIMESECONDS * realPower1;//added by DB 31.07.2018
  Ws2_calculated = LOOPTIMESECONDS * realPower2;//added by DB 31.07.2018
  Ws3_calculated = LOOPTIMESECONDS * realPower3;//added by DB 31.07.2018
...
void wh_accumulate_ws_reset() //added by DB 31.07.2018
{
  Wh1_accumulator = Wh1_accumulator + (Ws1_calculated / 3600);
  Wh2_accumulator = Wh2_accumulator + (Ws2_calculated / 3600);
  Wh3_accumulator = Wh3_accumulator + (Ws3_calculated / 3600);

  emontx.Wh1 = (int)(Wh1_accumulator + 0.5);
  emontx.Wh2 = (int)(Wh2_accumulator + 0.5);
  emontx.Wh3 = (int)(Wh3_accumulator + 0.5);
...
#ifdef CT4Phase
  Wh4_accumulator = Wh4_accumulator + (Ws4_calculated / 3600);
  emontx.Wh4 = (int)(Wh4_accumulator + 0.5);
  Ws4_calculated = 0.0;
#endif
...
void sendResults()
{
...
#ifdef CT4Phase
  Serial.print(F(",pf4:")); Serial.print(powerFactor4, 4);
#endif
  ...
  Serial.print(F(",Wh1:")); Serial.print(Wh1_accumulator, 1);
  Serial.print(F(",Wh2:")); Serial.print(Wh2_accumulator, 1);
  Serial.print(F(",Wh3:")); Serial.print(Wh3_accumulator, 1);
#ifdef CT4Phase
  Serial.print(F(",Wh4:")); Serial.print(Wh4_accumulator, 1);
#endif

  Serial.println();
  //delay(50); // why this delay in the original sketch?
#endif
