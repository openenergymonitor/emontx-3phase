...
struct eeprom {
  ...
  byte RF_freq;
  byte networkGroup;
  float vCal, i1Cal, i1Lead, i2Cal, i2Lead, i3Cal, i3Lead, i4Cal, i4Lead;
} data;

byte value;

byte signature = 035; //closest to OEM I could type as a byte :)
const int signature_EEPROM_location = 999; //of 1024 available on an ATMEGA 328P
...
static void load_config(bool verbose)
{
  ...
  if (EEPROM.read(signature_EEPROM_location) == signature)  //signature checking added by DB.
  {
...
static void save_config()
{
  ...
  */
  ...
  EEPROM.update(signature_EEPROM_location, signature);
...
static void wipe_eeprom(void)
{
  ...
  //for (byte j = 0; j < sizeof(data); j++)
    //EEPROM[j] = 255;
  EEPROM.update(signature_EEPROM_location, 255);
